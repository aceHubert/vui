...
export function parse(theme: Record<string, VuiThemeItem>, isItem = false): VuiParsedTheme {
...
export function genStyles(theme: VuiParsedTheme, cssVar = false): string {
  const { anchor, ...variant } = theme;
  const colors = Object.keys(variant);
