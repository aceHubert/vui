import { Vue, Component } from 'vue-property-decorator';

@Component({
  name: 'weui-demo-grid',
})
export default class DemoGrid extends Vue {
  icon: string =
    'data:image/png;base64,';

  render() {
    return (
      <div class="weui-demo demo-grid">
        <div>2 columns</div>
        <v-grid>
          {Array.from({ length: 2 }).map(() => (
            <v-grid-item icon={this.icon} label={'to cell'} link={{ name: 'demo-weui-cell' }}></v-grid-item>
          ))}
        </v-grid>
        <br />
        <div>3 * 3</div>
        <v-grid cols={3}>
          {Array.from({ length: 9 }).map((_, index) => (
            <v-grid-item icon={this.icon} label={'item' + (index + 1)}></v-grid-item>
          ))}
        </v-grid>
        <br />
        <div>4 columns</div>
        <v-grid cols={4}>
          {Array.from({ length: 4 }).map((_, index) => (
            <v-grid-item icon={this.icon} label={'item' + (index + 1)}></v-grid-item>
          ))}
        </v-grid>
        <br />
        <div>no left & right borders</div>
        <v-grid cols={4} showLrBorders={false}>
          {Array.from({ length: 4 }).map((_, index) => (
            <v-grid-item icon={this.icon} label={'item' + (index + 1)}></v-grid-item>
          ))}
        </v-grid>
        <br />
        <div>no top & bottom borders</div>
        <v-grid cols={4} showTbBorders={false}>
          {Array.from({ length: 4 }).map((_, index) => (
            <v-grid-item icon={this.icon} label={'item' + (index + 1)}></v-grid-item>
          ))}
        </v-grid>
        <br />
        <div>no item right border</div>
        <v-grid cols={4} showVerticalDividers={false}>
          {Array.from({ length: 4 }).map((_, index) => (
            <v-grid-item icon={this.icon} label={'item' + (index + 1)}></v-grid-item>
          ))}
        </v-grid>
        <br />
        <div>no item bottom border</div>
        <v-grid cols={3} showHorizontalDividers={false}>
          {Array.from({ length: 6 }).map((_, index) => (
            <v-grid-item icon={this.icon} label={'item' + (index + 1)}></v-grid-item>
          ))}
        </v-grid>
        <br />
        <div>custom</div>
        <v-grid cols={3}>
          {Array.from({ length: 6 }).map((_, index) => (
            <v-grid-item>
              <span style="text-align: center;display:block;">{index}</span>
            </v-grid-item>
          ))}
        </v-grid>
        <br />
        <br />
      </div>
    );
  }
}
