...
const components = [
  { icon, label: 'Actionsheet', link: '/actionsheet' },
  { icon, label: 'Alert', link: '/Alert' },
  { icon, label: 'Button', link: '/button' },
  { icon, label: 'Cell', link: '/cell' },
  { icon, label: 'Checker', link: '/checker' },
  { icon, label: 'Confirm', link: '/confirm' },
  { icon, label: 'Dialog', link: '/dialog' },
  { icon, label: 'Flexbox', link: '/flexbox' },
  { icon, label: 'v-grid', link: '/grid' },
  { icon, label: 'Loading', link: '/loading' },
  { icon, label: 'Loadmore', link: '/loadmore' },
  { icon, label: 'Navbar', link: '/navbar' },
  { icon, label: 'Panel', link: '/panel' },
  { icon, label: 'Popover', link: '/popover' },
  { icon, label: 'SearchBar', link: '/searchbar' },
  { icon, label: 'Slider', link: '/slider' },
  { icon, label: 'Switch', link: '/switch' },
  { icon, label: 'Toast', link: '/toast' },
];

@Component({
  name: 'weui-demo-index',
})
export default class DemoIndex extends Vue {
  render() {
    return (
      <div class="weui-demo demo-index">
        <v-grid cols={3}>
          {components.map((cmp) => {
            return <v-grid-item icon={cmp.icon} label={cmp.label} link={cmp.link}></v-grid-item>;
          })}
        </v-grid>
      </div>
    );
